...
**执行载体启动配置**必须**只追加项目定位信息**，并保留与 Agnir 无关的既有指令。持久 handoff 应刻意保持精简：通常只包含 canonical Project/ref，以及让新上下文读取根目录 `AGENTS.md` 的定位指令。不要把 Agnir 的 discovery、记忆权威、checkpoint 或仓库操作语义复制进 workspace / Project Instructions；这些语义继续属于 Project 本身。安装、迁移、兼容性晋升、升级或修复时，根目录 [`SKILL.md`](SKILL.md) 是面向智能体的权威分发操作流程。
...
稳定版发布仍是独立的权威 `main` 操作。release 暂存谱系只提供协调输入；已发布 tag 在接受后保持不可变。
