...
├── .github/
│   └── workflows/
│       └── conformance.yml                               # CI：self-host + Skill / activation / Core/profile/extension conformance
│
...
│   ├── agnir-0.1.md                                      # stable baseline + experimental VCS branch-continuity pressure map
...
│   ├── vcs_branch_continuity_reference.py                # branch snapshot / integration reconciliation / destination-ref verification model
...
│   ├── test_vcs_branch_continuity.py                     # real worktree isolation + integration/authority branch semantics
...
本页不是第二套协议或 Skill procedure。**Agent procedure 以根目录 `SKILL.md` 为准；Core 语义以 `spec/` 为准；repository/filesystem 行为以 `profiles/REPOSITORY_FILESYSTEM.md` 为准；实验性的 repository/VCS branch continuity 以 `profiles/VCS_BRANCH_CONTINUITY.md` 为准；机器可读 manifest 约束以 `schemas/` 为准；`history/` 仅保存 predecessor lineage。**
