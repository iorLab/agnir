...
├── .github/
│   └── workflows/
│       └── conformance.yml                               # CI：stable self-host + experimental VCS/Core 0.2 + full conformance
│
...
│   ├── CORE_0_2_DESIGN.md                                # Core 0.2 lineage identity / selection / integration design rationale
...
│   └── VCS_BRANCH_CONTINUITY.md                          # experimental branch-local continuity + merge/rebase/cherry-pick reconciliation extension
│
├── schemas/
...
│   ├── agnir-0.2-plan.md                                 # Core 0.2 dual-backend + migration conformance plan
...
│   ├── core_0_2_vcs_mapping_reference.py                 # VCS ref/SHA/failure mapping into generic Core 0.2 lineage semantics
...
│   ├── vcs_branch_continuity_reference.py                # VCS branch snapshot / staged integration / reconciliation / destination verification
...
│   ├── test_upgrade_semantics.py                         # compatible/no-op upgrade, stable-target, provenance preservation, migration classification
│   ├── test_core_0_2_parallel_continuity.py              # non-VCS logical lineage selection/isolation/integration/conflict tests
│   ├── test_core_0_2_vcs_mapping.py                      # VCS implementation mapped against generic Core 0.2 invariants
│   ├── test_core_0_2_migration.py                        # explicit/idempotent Core 0.1 → 0.2 migration preservation tests
...
│   ├── test_vcs_branch_continuity.py                     # real worktree isolation + staged target integration/authority branch semantics
...
本页不是第二套协议或 Skill procedure。**Agent procedure 以根目录 `SKILL.md` 为准；published Core `0.1` 语义以 `spec/AGNIR_CORE.md` 为准；实验 Core `0.2` 以 `spec/AGNIR_CORE_0_2_DRAFT.md` 为工作规范；repository/filesystem 行为以 `profiles/REPOSITORY_FILESYSTEM.md` 为准；实验性的 repository/VCS branch continuity 以 `profiles/VCS_BRANCH_CONTINUITY.md` 为准；机器可读 manifest 约束以 `schemas/` 为准；`history/` 仅保存 predecessor lineage。**
