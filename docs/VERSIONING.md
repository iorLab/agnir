...
- MINOR before 1.0: backward-compatible or intentionally pre-1.0 contract evolution, including substantial new capabilities. A feature such as Parallel Continuity may justify `v0.2.0` even if it remains behind an experimental profile.
...
- repository `v0.1.1` may expose Core `0.1`;
- repository `v0.2.0` may expose Core `0.2`;
- a future repository release could update packaging/docs while remaining compatible with the same Core line.
...
The normative release gate is defined in `docs/V1_RELEASE_CRITERIA.md`.
