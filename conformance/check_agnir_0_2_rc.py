...
SELF_LINEAGE_ID = "urn:agnir:lineage:v0.2.0-rc.1"
SELF_SELECTOR = "refs/heads/release/v0.2.0-rc.1"
REPOSITORY_VERSION = "0.2.0-rc.1"
SCHEMA = ROOT / "schemas" / "agnir-manifest-0.2.schema.json"


def main() -> None:
    # Preserve the established Agent-facing packaging and entry-point pressure.
    require_skill_package()
    ...
    for marker in (
        ...
        f'lineage: "{SELF_LINEAGE_ID}"',
        'branch_continuity: "lineage-bound"',
        f'selector: "{SELF_SELECTOR}"',
    ):
        if marker not in manifest:
            fail(f"RC AGNIR.yaml missing required marker: {marker}")
    if SELF_LINEAGE_ID == SELF_SELECTOR:
    ...
    # Core/profile 0.1 remains a supported compatibility line and regression surface.
    for path in (
        "spec/AGNIR_CORE.md",
        ...
        if not (ROOT / path).exists():
            fail(f"RC removed stable Core/profile 0.1 compatibility artifact: {path}")

    # Reuse the existing structural/activation guards; RC promotion must not regress them.
    require_readme_entry_guide(
        "README.md",
    ...
    print(
        f"PASS: Agnir RC self-host Core {snapshot.version} / {snapshot.profile} "
        f"lineage {snapshot.lineage_identity} repository {repository_version}"
    )
