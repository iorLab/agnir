...
Agnir `v1.0.0` stable is published and verified. Canonical brand integration, the Principal-approved launch/adoption strategy, the bilingual website source, and the first public-surface integration are authoritative on `main`. A second non-semantic rendering/localization repair is staged because the theme-aware SVG approach did not survive actual GitHub rendering.
...
2. **Finish the render-safe README/website repair.** Verify `maintenance/readme-render-language-cleanup` against latest `main`, run synthetic-merge conformance, integrate the coherent Project + continuity result, run post-merge conformance, and then retire the temporary staging branch. The intended public lockup is the approved self-contained `brand/exports/png/agnir-dark-usage.png`; do not revert to nested-reference SVGs on GitHub-hosted surfaces.
3. **Enable and verify GitHub Pages publication.** Repository host readback still reports `has_pages=false`. The current connector does not expose a Pages-settings mutation or workflow-dispatch action, and `actions/configure-pages` documented enablement requires a separate token with repository administration/pages-write authority rather than the ordinary `GITHUB_TOKEN`. Use an authority that can select **GitHub Actions** as the Pages publishing source, then run `Deploy Agnir website`, verify `https://iorlab.github.io/agnir/`, and verify English/Chinese pages plus favicon/social-card/logo loading before calling the site live.
4. **Complete the remaining Wave 0 repository-host surface.** The repository description is correct. Add the intended topics, confirm license/contribution presentation, provide an obvious external feedback path, and prepare stable public share surfaces. After Pages is live, set the repository homepage URL to the verified site.
...
10. **Retire completed temporary public-surface staging.** `maintenance/readme-website-public-surface` has no unique material product truth after PR #30 and may be deleted as ordinary housekeeping; also retire `maintenance/readme-render-language-cleanup` after its result is integrated.
11. **Keep brand production truth locked.** README, website, adoption/demo and social materials may consume canonical brand assets, but must not aesthetically reinterpret the approved Agnir identity. Material visual changes require new Principal approval.
12. **Run technical launch only after external proof exists.** Use the fresh-session value story first; use independent implementation, lineage/reconciliation, mount/storage relocation, and conformance as credibility layers after the user value is understood.
13. **Shift active product execution back to Svif where appropriate.** Svif's own canonical continuity governs its ChatGPT/Plugins publication path; do not smuggle Svif changes into Agnir maintenance.
14. **Keep FishUp production publication separate.** No FishUp main advancement without separate authorization.
...
- first README/website evidence: `.agnir/evidence/2026-09-07-readme-website-public-surface.md`;
- render-safe/localization follow-up will be recorded in `.agnir/evidence/2026-09-07-render-safe-public-surface.md`.
...
- Localized public prose should not accidentally mix display languages; exact identifiers/navigation remain allowed exceptions.
